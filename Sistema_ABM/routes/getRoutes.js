...
// Ruta para obtener usuarios filtrados por nickname
router.get('/obtener-usuarios-por-nickname', (req, res) => {
  const { nickname } = req.query;
  const query = 'SELECT * FROM usuarios WHERE nickname LIKE ?';
  connection.query(query, [`%${nickname}%`], (err, result) => {
    if (err) {
      console.error('Error al obtener usuarios por nickname:', err);
      res.status(500).json({ error: 'Error al obtener usuarios por nickname.' });
    } else {
